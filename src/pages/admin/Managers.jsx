  ...
  const { data: users, isLoading } = useQuery({
    ...
    queryFn: async () => {
      let q = supabase.from('users')
        ...
        .eq('role','manager').order('created_at',{ascending:false})
    ...
    <div>
      <div className="flex items-start justify-between mb-5">
        ...
        <Button onClick={() => setModal(true)} className="gap-2">
          <PlusCircle size={15}/> Add Manager
        </Button>
      </div>
      ...
      <Modal open={modal} onClose={() => setModal(false)} title="Create Manager">
        ...
        <div className="flex gap-2 justify-end mt-2">
          ...
          <Button loading={createUser.isPending} onClick={() => createUser.mutate()}>Create Manager</Button>
        </div>
      </Modal>
    </div>
  )
