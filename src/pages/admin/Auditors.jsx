  ...
  const createUser = useMutation({
    mutationFn: async () => {
      ...
      const { error } = await supabase.from('users').insert({
        first_name: form.first_name, last_name: form.last_name, email: form.email, phone: form.phone, password_hash: hashed, role: 'auditor', status: 'active', staff_code: staffCode,
      })
      if (error) throw error
    },
    onSuccess: () => {
      ...
      setFlash({ type:'success', msg:'Auditors created.' })
    },
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            ...
            {!isLoading && !users?.length && (
              <tr><td colSpan={6} className="px-4 py-12 text-center text-gray-400">No auditors yet.</td></tr>
            )}
          </tbody>
        </table>
      </div>
