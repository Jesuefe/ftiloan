  ...
  const createUser = useMutation({
    mutationFn: async () => {
      ...
      const { error } = await supabase.from('users').insert({
        ...
        password_hash: hashed, role: 'operator', status: 'active', staff_code: staffCode,
      })
      if (error) throw error
    },
    onSuccess: () => {
      ...
      setFlash({ type:'success', msg:'Operator created.' })
    },
  ...
  return (
    <div>
      ...
      <div className="space-y-2 lg:hidden">
        ...
        {!isLoading && !users?.length && <div className="py-12 text-center text-sm text-gray-400 bg-white rounded-2xl border border-gray-200">No operators yet</div>}
      </div>
      ...
      <div className="hidden lg:block bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            ...
            {!isLoading && !users?.length && <tr><td colSpan={6} className="px-4 py-12 text-center text-gray-400">No operators yet.</td></tr>}
          </tbody>
        </table>
      </div>
