...
export default function AdminSalary() {
  ...
  const { data: monthlyRecovery } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data } = await supabase.from('repayment_schedule')
        .select('amount_due').in('status',['pending','overdue','partial'])
  ...
  const save = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey:['staff-salaries'] }); setEditing(null); setSalaryVal(''); setFlash({ type:'success', msg:'Salary updated.' }) },
  ...
  return (
    <div>
      <div className="mb-5">
        <h1 className="text-xl font-bold flex items-center gap-2"><DollarSign size={20} className="text-brand-600"/> Salary Management</h1>
      ...
      {/* Summary — 3 cols on desktop, stack on mobile */}
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-5">
        {[
          { label:'Expected Recovery', value: formatNaira(monthlyRecovery||0), icon: TrendingUp, color:'green' },
          { label:'Total Salaries',    value: formatNaira(totalSalary),         icon: TrendingDown, color:'red' },
          { label: surplus>=0 ? 'Surplus' : 'Deficit', value: formatNaira(Math.abs(surplus)), icon: DollarSign, color: surplus>=0?'green':'red' },
      ...
      {/* Staff list — cards on mobile */}
      <div className="space-y-2 lg:hidden">
        {staff?.map(u => (
          <div key={u.id} className="bg-white rounded-2xl border border-gray-200 p-4 flex items-center justify-between">
            <div>
              <div className="font-medium text-gray-900">{u.first_name} {u.last_name}</div>
              <div className="text-xs text-gray-400 capitalize">{u.role?.replace(/_/g,' ')} · {u.staff_code||'—'}</div>
              <div className={`text-sm font-bold mt-1 ${Number(u.salary)>0?'text-gray-900':'text-gray-400'}`}>
                {Number(u.salary)>0 ? formatNaira(u.salary) : 'Not set'}
              </div>
            </div>
            <Button size="sm" variant="outline" onClick={() => { setEditing(u); setSalaryVal(u.salary||'') }}>Edit</Button>
          </div>
        ))}
      </div>
            ...
            <tbody>
              {staff?.map(u => (
                <tr key={u.id} className="border-b border-gray-100 hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3"><span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-gray-100 text-gray-700 capitalize">{u.role?.replace(/_/g,' ')}</span></td>
                  <td className="px-4 py-3 font-bold">{Number(u.salary)>0 ? formatNaira(u.salary) : <span className="text-gray-400">Not set</span>}</td>
                  <td className="px-4 py-3"><Button size="sm" variant="outline" onClick={() => { setEditing(u); setSalaryVal(u.salary||'') }}>Edit</Button></td>
                </tr>
              ))}
            </tbody>
          </table>
